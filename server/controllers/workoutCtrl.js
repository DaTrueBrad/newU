...
module.exports = {
  ...
  getCurrent: async (req, res) => {
    const workout = await sequelize.query(
      `SELECT w.id, w.name, w.data FROM workouts w, users u WHERE u.id = ${req.query.id} AND u.current = w.id`
    );
    console.log(workout);
  ...
  selectCurrent: async (req, res) => {
    ...
    await sequelize.query(
      `UPDATE users SET current = ${req.body.id} WHERE id = ${req.body.user}`
    );
  ...
  getTemplates: async (req, res) => {
    let templates = await sequelize.query(`SELECT * FROM workouts WHERE created_by = 1`);
